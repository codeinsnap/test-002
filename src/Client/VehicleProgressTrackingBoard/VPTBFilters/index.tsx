...
import { dashboardHeaderStyle } from "../../Layout/DashboardHeader/dashboard_header_tailwind";
...
import { TSelectedFilters } from "../GnattChart/gnatt_chart_types";
...
export default function VPTBFilters({ handleResetFilter, setFilters, applyFilters } : TVPTBFiltersProps) {
  ...
  const apiData = response.vehicleFilterData
  ...
  const sortFilterValues = (x: SelectData[]) => {
    return x.sort((p1: SelectData, p2: SelectData) => {
      ...
      const a = p1.name;
      const b = p2.name;
      const aMatch = alphanumericRegex.exec(a);
      const bMatch = alphanumericRegex.exec(b);
      if (aMatch && bMatch) {
  ...
  useEffect(() => {
    populateSelectData(apiData);
  }, [selectedLines, selectedStalls, selectedVins, selectedMembers, selectedStatus, apiData]);
    ...
    switch (name) {
      case 'line':
        if (checked) {
          ...
          setSelectedStalls(selectedStalls.filter(stall => !stall.startsWith(value + '_')));
        }
        break;
      case 'stall':
        if (checked) {
  ...
  return (
    <div className={vptb_filters_tailwind.filterWrapper} key={refresh}>
